...
def get_blacklist_status(operator_id: int) -> bool:
    ...
    with orm.db_session:
        counts = orm.select(
            b for b in Blacklist if b.operator_id == operator_id).count()
        if counts >= 1:
            latest_blacklist_log = orm.select(b for b in Blacklist if b.operator_id == operator_id).order_by(
                orm.desc(Blacklist.time)).limit(1)[0]
            current_status = latest_blacklist_log.status
    return current_status


class BlacklistParam:
    def __init__(self, operator_id: int, current_status: bool, command_name: str, message_id: int, time: int, group_id: int) -> None:
...
async def depend(event: GroupMessageEvent):
    ...
    return BlacklistParam(operator_id,
                          current_status,
                          command_name,
                          message_id,
                          time,
...
async def _(bot: Bot, param: BlacklistParam = Depends(depend)):
    if param.command_name == "status":
        message = (f"[CQ:reply,id={param.message_id}]"
                   f"您的目前聊天记录收集黑名单状态为：{'开启' if param.current_status else '关闭'}"
                   "\n"
